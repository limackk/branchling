...
- [ ] A `--plan` run that stops on a wave somebody else is working says so, and
      does not call the queue empty. [proof: plan-order]
- [ ] A `--plan` run that stops with unplanned open work left says how much was
      left alone. [proof: plan-order]
- [ ] Without `--plan` the report is byte for byte what it was. [proof: suite-green]
