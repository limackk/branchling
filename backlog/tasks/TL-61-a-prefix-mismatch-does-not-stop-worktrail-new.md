...
verification:
  ...
  - bash: "d=$(mktemp -d); T="$PWD/bin/branchling.mjs"; node $T init --dir \"$d\" >/dev/null; node $T new --dir \"$d\" --title First >/dev/null 2>&1; sed -i '' 's/^task_id_prefix: .*/task_id_prefix: OTHER/' \"$d/config.yaml\"; node $T new --dir \"$d\" --title Second >/dev/null 2>&1 && { echo 'new still writes despite the mismatch'; exit 1; }; echo 'new refuses on a mismatch — OK'"
---
