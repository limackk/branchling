...
| Check | Source |
|---|---|
| `config.yaml` parses, keys are known | [TL-60](TL-60-a-typo-in-config-yaml-sails-through-build-and-check.md) |
| vocabularies are consistent (`archived_statuses` ⊆ `statuses`, etc.) | works today, collect it |
| values in tasks fit within the vocabularies | [TL-56](TL-56-the-types-vocabulary-in-config-yaml-has-drifted-from-the.md) |
| the prefix from the configuration matches the tree | [TL-61](TL-61-a-prefix-mismatch-does-not-stop-worktrail-new.md) |
| views are ignored by git | [TL-59](TL-59-co-location-views-are-not-ignored-in-an-existing-repo.md) |
| `history/*.jsonl` has `merge=union` | TL-59 |
| backlog guards (`check`) | exists |
| hooks installed or not | [TL-46](TL-46-worktrail-init-hooks-a-gate-that-installs-itself-for-the.md) |
