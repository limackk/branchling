...
- 2026-09-01 pending — agent:claude — **name changed `tasklog` → `worktrail`** (decision and reasoning in the log of [TL-20](TL-20-close-the-tool-s-name-before-open-source-publication.md)). What this changes in THIS task:
  **Step 2 done and produced a NEGATIVE result.** The claim "a name change is one edit" is FALSE — measured, not estimated: 158 files, including 26 in `scripts/`. `scripts/product.mjs` exports `PRODUCT_NAME`, but the rest of the code does not use it. The finding was moved out to [TL-117](TL-117-product-name-from-a-single-constant-not-from-a-literal.md), because it is work, not a decision — this task stays a decision task.
  ...
  **Raised risk:** the name is freshly chosen and still unreserved, so the window between choosing it and publishing is now the only safeguard — and it is open.
  The `## Context` above mentions `"private": true` and `"license": "UNLICENSED"` — **both are stale** regardless of the name (the license was settled in [TL-48](TL-48-license-and-package-metadata-today-nobody-can-use-this.md), `private` removed).
