...
function conformance() {
  if (scenario === "cancelled") {
    ...
    child.kill();
    child.on("exit", () => console.log(JSON.stringify({ version: 1, scenario, outcome: scenario, childPid: child.pid })));
    return true;
  }
  if (env[PREFIX + "CONFORMANCE"] === "1") {
    console.log(JSON.stringify({ version: 1, scenario, outcome: scenario }));
    return true;
  }
  return false;
}
