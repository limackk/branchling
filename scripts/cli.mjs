...
export const COMMANDS = {
  ...
  log: {
    ...
    usage: [
      `${N} log <ID> [--limit <n>] [--decisions] [--json] [--dir <path>]`,
      "",
      ...
      "  --decisions        only what was DECIDED and what is still unanswered. Reading",
      `                     a task's decisions is a mode of \`${N} log\`, not of`,
      "                     `decide` — that one records them, and a read inferred from",
      "                     a MISSING `--reason` would print where it now refuses",
  ...
  next: {
    ...
    usage: [
      ...
      "                     the order the wave LISTS decides, outranking the policy",
      "                     above — a wave is an order, and its sequence is the only",
      "                     place an author can say that one member reads another's",
      "                     correction. Without a plan file it FAILS rather than matching",
  ...
  decide: {
    ...
    usage: [
      `${N} decide <ID> --reason "…" [--resolves <event id>] [--actor <ns:name>] [--json] [--dir <path>]`,
      `${N} decide <ID> --resolves <event id> --choose <n> [--actor <ns:name>] [--json] [--dir <path>]`,
      "",
      ...
      "  --json             the decision JUST RECORDED, and what remains unanswered after",
      "                     it, for a program. It reports a write; it is not a way to ask",
      `                     what was decided earlier — that is \`${N} log <ID> --decisions\`.`,
      "                     This command has no reading form, because the only thing that",
      "                     could select one is a MISSING `--reason`, and a dropped flag",
      "                     would then print and exit 0 where it refuses (TL-396)",
      "",
  ...
  run: {
    ...
    usage: [
      ...
      "",
      "  AN AGENT THAT REPEATS ITSELF IS REPORTED AS SUCH. When every attempt on one",
      "  task printed byte-identical, non-empty output, the report says so and quotes",
      "  what the hand said: a hand that is working reads a tree the previous attempt",
      "  moved, so it does not answer twice the same way. That is EVIDENCE, not a",
      "  diagnosis — nothing is read from the wording, and a vendor cutting your agent",
      "  off looks exactly like a hand that gives up the same way every time. The",
      "  ending, the attempts and the parked reason are unchanged; the reason gains the",
      "  observation so `no verification after N agent attempts` is not read as a",
      "  finding about work nobody measured.",
      "",
      "  WHATEVER THE HAND WROTE BEFORE STOPPING STAYS IN YOUR TREE, uncommitted, and it",
      "  is yours. This command never commits, reverts or cleans, so a half-written",
      "  deliverable left by an agent that stopped is one `git add -A` away from an",
      "  unrelated commit. `resume <ID>` lists a task's uncommitted work.",
      "",
