...
export const COMMANDS = {
  ...
  query: {
    ...
    usage: [
      ...
      "",
      "  --tasks <path>          read the task files from this directory rather than the",
      "                          one --dir resolves to. It names ONE backlog, so it cannot",
      "                          be combined with --all-projects.",
      "",
      ...
      "",
      "  --all-projects          the same question asked of EVERY registered backlog",
      "                          (`project list` shows which). It is a VIEW: N directories",
      "                          read and assembled in memory, with no storage of its own —",
      "                          deleting the registry costs this flag and nothing else.",
      "",
      "                          Every row names its project, and the identity is the PAIR",
      "                          (project, id): numbers are unique WITHIN a project, so two",
      "                          backlogs both holding a task 12 is the normal case rather",
      "                          than a clash to resolve.",
      "",
      "                          A registered project whose directory has moved is reported",
      "                          IN THE RESULT, `--json` included, and the pass continues.",
      "                          A silent skip would turn `you moved the repository` into",
      "                          `that project has no tasks`, and a consumer would count an",
      "                          incomplete set as complete.",
      "",
      "                          A filter value is refused only if NO project has heard of",
      "                          it: judging against one project's vocabulary would refuse a",
      "                          query that is meaningful in the second.",
      "",
      "                          It cannot be combined with `--dir` or `--tasks`, which name",
      "                          one backlog.",
    ].join("\n"),
  },
  build: {
  ...
  history: {
    ...
    usage: [
      `${N} history [--file <task.md>] --actor <ns:name> [--source s] [--reason "…"] [--attribute] [--event <id>]`,
      "",
      ...
      "  With one candidate, `--attribute` is enough. With several, the command refuses",
  ...
  new: {
    ...
    usage: [
      ...
      `          [--owner <o>] [--epic <e>] [--estimate <2h>] [--wave "<name>"] [--dir <path>]`,
      "",
  ...
  actors: {
    script: "actors.mjs",
    summary: "an actor's record, added up from the log — closings, first pass, rework, handbacks",
    usage: [
      ...
      "    reopened     closings that came back. The number is `audit`'s, not a second",
      "                 fold of the same log — two folds would eventually disagree",
      "                 about the same week and a reader could not tell which lied",
      ...
      "  all predates them and lands in `legacy`; `unknown` keeps its own row.",
      "",
      ...
      "  that keeps producing rework.",
      "",
      "  `actor_policy_priorities` and `actor_policy_min_first_pass` in config.yaml",
      `  turn the report into a POLICY: \`${N} next\` then withholds work at those`,
      "  priorities from an actor whose record is below the threshold. It filters",
      "  eligibility and never reorders — the order is the dispatcher's.",
      "",
  ...
  project: {
    ...
    usage: [
      ...
      `${N} project prune [--dry-run] [--all-unavailable] [--json]`,
      "",
      ...
      "",
      "  PRUNE — forget the entries whose DIRECTORY no longer exists. Never automatic",
      "  and never on a schedule: this file records a decision you made, and a network",
      "  share that did not mount this morning is not a reason to forget a project. So",
      "  it is a command you run, `--dry-run` first, and it names every entry it takes",
      "  rather than counting them — this is the only record that a directory was ever",
      "  a project of yours.",
      "",
      "  A directory that EXISTS but is no longer a backlog is left alone, because that",
      "  is somebody's checkout to look at rather than a project that went away.",
      "  `--all-unavailable` includes those too.",
      "",
      "  exit: 0 done · 1 refused · 2 usage error",
    ].join("\n"),
  },
  ...
  profile: {
    ...
    usage: [
      `${N} profile create <name> --adapter <command> (--prompt <text>|--prompt-file <path>)`,
      `                     [--model <name>] [--effort <value>]`,
      `${N} profile list [--json]`,
      `${N} profile show <name> [--json]`,
      `${N} profile update <name> [--adapter <command>] [--model <name>] [--effort <value>]`,
      `                     [--prompt <text>|--prompt-file <path>]`,
      `${N} profile remove <name> [--json]`,
      "",
      "  Profiles are local user data: the same repository may be run through different",
      "  providers by different contributors. `adapter` is an opaque command, not a closed",
      "  provider list. Credentials are refused here; refer to environment variables or",
  ...
  quote: {
    script: "quote-command.mjs",
    summary: "what this task will probably cost, before it is handed to an agent",
    usage: [
      `${N} quote <ID> [--json] [--dir <path>]`,
      "",
      "  The inverse of `stats --calibration`: that one asks what tasks of this size",
      "  HAVE cost, this one applies the answer to one task. It computes nothing of",
      "  its own — the same buckets, the same threshold, the same refusals — because",
      "  two answers to one question would leave nothing to say which was right.",
      "",
      "  The bucket is estimate x type, falling back to the estimate alone when that",
      "  cell is too thin. THE FALLBACK IS PRINTED: one nobody can see is",
      "  indistinguishable from a narrower answer than the tool actually has.",
      "",
      "  Below `min_report_n` the answer is `not enough data`, never a number. The",
      "  answer is always a RANGE (p20-p80), never a point. Tokens are the primary",
      "  axis and the dollar amount is a conditional derivative — no cost adapter",
      "  means no token column, not a column of zeroes, and a subscription gives",
      "  tokens with no amount rather than a fictitious one.",
      "",
      "  Models are never averaged together: tokens of two models are two different",
      "  units of effort, so each is its own cell and a thin one says so.",
      "",
      "  The share of minutes no leg of the attribution chain could place travels",
      "  WITH the forecast. A number that hides how much of its input was guesswork",
      "  is a metric asserting its own trustworthiness.",
      "",
      "  exit: 0 answered (`not enough data` IS an answer) · 1 no such task ·",
      "        2 usage error, a task with no estimate included",
    ].join("\n"),
  },
  stats: {
  ...
  run: {
    ...
    usage: [
      ...
      "",
      "  THE HAND IS TOLD WHO IT IS. Every agent command runs with the actor this run",
      "  claimed the task under, the role it is serving, the task id and the data",
      "  directory in its environment — `<PRODUCT>_ACTOR`, `_ROLE`, `_TASK` and `_DIR`,",
      "  the product name upper-cased — so `handoff`, `ask` and `decide` can be run",
      "  under the actor that actually holds the task.",
      "",
      "  `--agent` IS OPTIONAL WHEN A ROLE IS SERVED. A run of specialists needs no",
      "  generalist: with `--agent-for` given and `--agent` absent, a task that asks for",
