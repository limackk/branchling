...
async function main(argv) { let p;try{p=parse(argv);}catch(e){console.error(failure(N+" runs",e.message,[],[N+" runs --help"]));return 2;} const root=resolveBacklogDir({dir:p.dir||undefined,moduleDir:HERE}).root; const one=()=>readExecutionRecord(root,p.id); if(p.action==="list"){const runs=listExecutionRecords(root).filter(r=>r.kind==="run");if(p.json)printJson("runs",{runs});else console.log(runs.map(r=>r.id+"  "+r.phase).join("\n")||"no local runs");return 0;} let r=one();if(!r){if(p.json)printJson("run",{run:null,alive:null});else console.error(failure(N+" runs","no such local run: "+p.id,[]));return 1;} if(p.action==="wait"){const until=Date.now()+p.timeout*1000;while(!["finished","cancelled","interrupted"].includes(r.phase)&&Date.now()<until){await new Promise(x=>setTimeout(x,200));r=one()||r;} } if(p.action==="cancel"&&! ["finished","cancelled","interrupted"].includes(r.phase)){if(!alive(r)){updateRunRecord(root,r,{phase:"interrupted",outcome:"stale-supervisor"});}else{try{process.kill(-r.supervisor.pid,"SIGTERM");}catch{process.kill(r.supervisor.pid,"SIGTERM");}updateRunRecord(root,r,{phase:"cancelled",outcome:"cancelled"});}} if(p.json)printJson("run",{run:r,alive:alive(r)});else console.log(r.id+"  "+r.phase+(alive(r)?"  alive":""));return 0; }
